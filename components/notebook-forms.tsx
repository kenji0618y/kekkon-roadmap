...
import {ArrowUpRight,BookOpen,CalendarDays,Check,ClipboardCopy,Heart,Info,ShieldCheck,Sparkles} from 'lucide-react';
...
import {eligibilityNote,emptyRecord,memorySchema,profileSchema,recordSchema,statusNames,type Memory,type Profile,type Task,type TaskRecord} from '../lib/model';
import {sources} from '../data/catalog';
import {shortDate,statutoryDeadline,todayJapan} from '../lib/dates';
...
export function TaskForm({task:t,profile:p,record,onSave,busy,onDirty,hasBook,onProfile}:{task:Task,profile:Profile,record?:TaskRecord,onSave:(r:TaskRecord)=>Promise<void>,busy:boolean,onDirty:(v:boolean)=>void,hasBook:boolean,onProfile:()=>void}){
 ...
 const copyQuestions=async()=>{try{await navigator.clipboard.writeText(`${t.title}\n\n${t.questions.map((q,i)=>`${i+1}. ${q}`).join('\n')}`);toast.success('質問メモをコピーしました');}catch{toast.error('コピーできませんでした。表示された質問を選択してコピーしてください。');}};
