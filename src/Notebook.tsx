...
import {clearGrokLocalOnly,GROK_MODE_EVENT,markGrokLocalOnly,readGrokLocalOnlyFlag} from './lib/grok-mode';
...
export default function FutureNotebook(){
 ...
 const [localOnlyMode,setLocalOnlyMode]=useState(()=>!loadGrokKey()||!!readGrokLocalOnlyFlag());
 ...
 useEffect(()=>{
  const sync=()=>setLocalOnlyMode(!loadGrokKey()||!!readGrokLocalOnlyFlag());
  sync();
  window.addEventListener(GROK_MODE_EVENT,sync);
  window.addEventListener('storage',sync);
  return()=>{window.removeEventListener(GROK_MODE_EVENT,sync);window.removeEventListener('storage',sync);};
 },[]);
 useEffect(()=>{
