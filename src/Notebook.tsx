...
import {DEFAULT_GROK_BASE,askGrokResearch,grokErrorJa,NETWORK_ERROR_JA,GROK_CREDITS_CONSOLE_URL,GROK_CREDITS_LIMIT_JA,isGrokCreditsLimitResult,loadGrokBase,loadGrokKey,saveGrokBase,saveGrokKey} from './lib/amity-grok';
...
export default function FutureNotebook(){
 ...
 const runSyncAction=async(fn:()=>Promise<unknown>)=>{setSyncBusy(true);try{persistSync();const result=await fn();if(typeof result==='string')toast.success(result);}catch(e){toast.error(e instanceof TypeError?NETWORK_ERROR_JA:e instanceof Error?e.message:'同期に失敗しました',{duration:8000});}finally{setSyncBusy(false);}};
 ...
 <section id="deadline-block-institutional" className="deadline-block" aria-label="制度の期限">
  <details className="deadline-secondary-fold" open>
   <summary>
    <h2 className="deadline-block-label">制度の期限（一覧）</h2>
    <span className="hint">カレンダーと同じ締切を、リストでも確認</span>
   </summary>
 ...
 <TabsContent value="settings" className="tab-surface"><SectionTitle eyebrow="設定" title="ふたりらしい手帳に。" sub="呼び名、共有、バックアップ。いつでも整え直せます。"/>
 ...
 <section id="settings-backup" className="paper-card settings-card"><div className="settings-icon"><ArrowDownToLine/></div><h2>保存と、二人での受け渡し。</h2><p>記録（進みぐあい・メモ・金額）は、この端末のブラウザの中に保存されます。二人で同じ手帳を使うときや端末を変えるときは、バックアップのファイルを書き出して、もう一方の端末で読み込んでください。</p><p className="hint">バックアップには、名前・日付・メモが入ります。渡す相手と保管先を選んでください。</p><input ref={uploadRef} type="file" accept=".json,application/json" className="sr-only" aria-label="手帳のバックアップ" onChange={e=>{const file=e.target.files?.[0];if(file)void loadBackup(file);e.target.value='';}}/><div className="stack-actions"><Action secondary onClick={exportBackup} disabled={!data.book}><Download/>バックアップを書き出す</Action><Action secondary onClick={()=>uploadRef.current?.click()}><ArrowUpRight/>バックアップを読み込む</Action><Action secondary onClick={()=>window.print()}><Printer/>手帳を印刷・PDFにする</Action></div><p className="hint">以前の版で書き出したファイルも読み込めます（自動では移りません）。</p></section>
 <section id="settings-research" className="paper-card settings-card"><div className="settings-icon"><Search/></div><h2>今の制度を調べる</h2><p>気になる制度・手続きを入力すると、AmityちゃんがAIで短く調べます。「探す」タブで、同じ言葉の項目も見られます。</p><div className="field" style={{width:'100%',marginBottom:12}}><Label htmlFor="policy-research-q">調べたいこと</Label><Input id="policy-research-q" value={researchQ} onChange={e=>setResearchQ(e.target.value)} placeholder="例：広島市 児童手当 申請、転入届の期限…" maxLength={200} autoComplete="off" disabled={researchBusy}/></div><div className="stack-actions"><Action secondary disabled={researchBusy||!researchQ.trim()} onClick={()=>void(async()=>{const q=researchQ.trim().slice(0,200);if(!q)return;setResearchBusy(true);setResearchOut('');try{const r=await askGrokResearch(q);if(r.ok)setResearchOut(r.text);else if(r.error==='no-key'){markGrokLocalOnly('no-key');setResearchOut('AIのキーがありません。下の「詳細設定」でキーを入れるか、まず「探す」タブで確認してね。');toast.info('キーがないため「探す」タブを開きます');setQuery(q);setTab('find');}else if(isGrokCreditsLimitResult(r.error)){markGrokLocalOnly('credits-limit');const local=answerDeskQuery(q,3,p);setResearchOut(`${GROK_CREDITS_LIMIT_JA}\n\n—— 手帳の中の案内 ——\n${local.text}`);toast.error(GROK_CREDITS_LIMIT_JA,{duration:8000});}else{setResearchOut(`調べられなかったよ。${grokErrorJa(r.error)} 公式案内もあわせて確認してね。`);toast.error(grokErrorJa(r.error));}}finally{setResearchBusy(false);}})()}>{researchBusy?<><LoaderCircle className="spin" size={16}/>調べています…</>:<><Sparkles size={16}/>Amityに調べてもらう</>}</Action><Action secondary disabled={!researchQ.trim()} onClick={()=>{setQuery(researchQ.trim());setTab('find');}}>「探す」で見る<ArrowRight/></Action></div>{researchOut&&<div className="research-result" role="status"><p className="hint" style={{marginBottom:6}}>Amityの調べメモ（金額は勝手に作りません。結婚新生活支援は広島市では受けられません）</p><p style={{whiteSpace:'pre-wrap',fontSize:13,lineHeight:1.55,margin:0}}>{researchOut}</p></div>}<p className="hint" style={{marginTop:10}}>{loadGrokKey()?'AIで調べられます。':'AIのキーがなくても「探す」タブは使えます。'}</p></section>
 <section className="paper-card settings-card danger-reset-card"><div className="settings-icon"><Trash2/></div><h2>スタンプ進捗をリセット</h2><p>各項目のスタンプ状態・金額・メモだけを消します。プロフィールと記念手帳の文章は残します。バックアップを先に書き出すことをおすすめします。</p><p className="hint">誤タップ防止のため、確認ダイアログで「リセット」と入力し、もう一度チェックを入れる必要があります。</p><div className="stack-actions"><Action secondary onClick={()=>{setResetTyped('');setResetAck(false);setResetOpen(true);}} disabled={!data.book||data.busy}><Trash2/>スタンプ進捗をリセット…</Action></div></section>
 <section id="settings-advanced" className="paper-card settings-card settings-advanced"><details className="settings-advanced-fold" id="settings-advanced-fold"><summary><span className="settings-icon"><Settings2/></span><span><h2>詳細設定</h2><span className="hint">ふだんは開かなくて大丈夫です（端末どうしの自動同期・AIのキー・毎年の見直しメモ）</span></span></summary>
 <div id="settings-gist-sync" className="settings-sub"><h3><Cloud size={17}/>端末どうしの自動同期</h3><p>二人の端末で同じ手帳を自動でそろえたいときに使います。GitHub のアカウントと、GitHub で発行する「アクセス用のキー（gist の権限つき）」が必要です。使わなくても、上のバックアップのファイルで受け渡しできます。</p><p className="hint">キーはこの端末の中だけに保存されます。二人の端末それぞれで、同じ同期先とキーを入れてください。</p><label className="scope-toggle" style={{marginBottom:14}}><Switch checked={syncEnabled} onCheckedChange={v=>{setSyncEnabled(v);data.saveSyncSettings({token:syncToken,gistId:syncGistId||DEFAULT_GIST_ID,enabled:v});}}/><span>自動同期を使う</span></label><div className="field" style={{width:'100%',marginBottom:12}}><Label htmlFor="gist-pat">アクセス用のキー（GitHub）</Label><Input id="gist-pat" type="password" autoComplete="off" value={syncToken} onChange={e=>setSyncToken(e.target.value)} onBlur={()=>persistSync({gistId:syncGistId||DEFAULT_GIST_ID})} placeholder="ghp_… または github_pat_…" maxLength={200}/></div><div className="field" style={{width:'100%',marginBottom:12}}><Label htmlFor="gist-id">同期先のID（変更できます）</Label><Input id="gist-id" value={syncGistId} onChange={e=>setSyncGistId(e.target.value)} onBlur={()=>persistSync({gistId:syncGistId||DEFAULT_GIST_ID})} placeholder={DEFAULT_GIST_ID} maxLength={64} autoComplete="off"/><p className="hint" style={{marginTop:6}}>初期値 {DEFAULT_GIST_ID}</p></div><p className="hint">状態：{syncStatusLabel} · 最終同期：{lastSyncLabel}</p><div className="stack-actions"><Action secondary disabled={syncBusy||!syncToken.trim()} onClick={()=>void runSyncAction(async()=>{persistSync({token:syncToken,gistId:syncGistId||DEFAULT_GIST_ID,enabled:syncEnabled});const id=await data.createGistNow();setSyncGistId(id);setSyncEnabled(true);return `同期先を新しく作りました（${id.slice(0,8)}…）`;})}>同期先を新しく作る</Action><Action secondary disabled={syncBusy||!syncToken.trim()||!(syncGistId||DEFAULT_GIST_ID).trim()} onClick={()=>void runSyncAction(()=>{persistSync({gistId:syncGistId||DEFAULT_GIST_ID});return data.syncNow();})}>今すぐ同期</Action><Action secondary disabled={syncBusy||!syncToken.trim()} onClick={()=>void runSyncAction(()=>{persistSync({gistId:syncGistId||DEFAULT_GIST_ID});return data.testSync();})}>つながるか試す</Action></div><p className="hint">同期先は、公開されない GitHub の Gist（futari-miraicho.json）です。</p></div>
 <div id="settings-grok" className="settings-sub"><h3><Sparkles size={17}/>AIのキー（Amityちゃんの調べもの）</h3><p>Amityちゃんは、手帳の内容から探すのに加えて、AI（xAI の Grok）で調べた答えを足せます。ふだんは何も入れなくても使えます。自分のキーを使いたいときだけ入れてください。キーはこの端末の中だけに保存されます。</p><div className="field" style={{width:'100%',marginBottom:12}}><Label htmlFor="amity-grok-key">xAI のキー</Label><Input id="amity-grok-key" type="password" autoComplete="off" value={grokKey} onChange={e=>setGrokKey(e.target.value)} onBlur={()=>{saveGrokKey(grokKey);if(grokKey.trim()){clearGrokLocalOnly();}else{markGrokLocalOnly('no-key');}toast.success(grokKey.trim()?'AIのキーをこの端末に保存しました':'AIのキーを消しました');}} placeholder="キーを貼り付け（任意）" maxLength={200}/></div><div className="field" style={{width:'100%',marginBottom:12}}><Label htmlFor="amity-grok-base">接続先のURL（ふだんは変えません）</Label><Input id="amity-grok-base" value={grokBase} onChange={e=>setGrokBase(e.target.value)} onBlur={()=>{saveGrokBase(grokBase||DEFAULT_GROK_BASE);setGrokBase(loadGrokBase());}} placeholder={DEFAULT_GROK_BASE} maxLength={200} autoComplete="off"/><p className="hint" style={{marginTop:6}}>初期値 {DEFAULT_GROK_BASE}</p></div>
...
<div className="stack-actions" style={{marginTop:10}}><Action secondary onClick={()=>window.open(GROK_CREDITS_CONSOLE_URL,'_blank','noopener,noreferrer')}><ExternalLink size={16}/>利用枠を増やす（xAIコンソール）</Action></div></div>
...
<details className="pair-legacy-fold">
 ...
 <p className="hint">ファイルを渡さずに自動でそろえたいときは、設定の「詳細設定」にある「端末どうしの自動同期」を使えます（GitHub のアカウントが必要です）。</p>
