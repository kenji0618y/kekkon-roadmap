...
export function Choice({label,value,onChange,options,disabled=false}:{label:string,value:string,onChange:(v:string)=>void,options:Record<string,string>,disabled?:boolean}){const id=useId();return <div className="field"><Label htmlFor={id}>{label}</Label><Select value={value} onValueChange={onChange} disabled={disabled} modal={false}><SelectTrigger id={id} className="field-control"><SelectValue/></SelectTrigger><SelectContent position="popper">{Object.entries(options).map(([v,t])=><SelectItem key={v} value={v}>{t}</SelectItem>)}</SelectContent></Select></div>;}
