...
export function Choice({label,value,onChange,options,disabled=false,triggerClassName=''}:{label:string,value:string,onChange:(v:string)=>void,options:Record<string,string>,disabled?:boolean,triggerClassName?:string}){const id=useId();return <div className="field"><Label htmlFor={id}>{label}</Label><Select value={value} onValueChange={onChange} disabled={disabled}><SelectTrigger id={id} className={`field-control${triggerClassName?` ${triggerClassName}`:''}`}><SelectValue/></SelectTrigger><SelectContent position="popper">{Object.entries(options).map(([v,t])=><SelectItem key={v} value={v} className={v==='done'?'status-opt-done':v==='na'?'status-opt-na':undefined}>{t}</SelectItem>)}</SelectContent></Select></div>;}
