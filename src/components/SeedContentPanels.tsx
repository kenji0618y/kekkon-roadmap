    ...
    <section className="seed-block institutional-deadlines" aria-label="制度・カレンダー締切">
      ...
      <div className="seed-deadline-list">
        {abs.map((d) => {
          ...
          return (
            <article key={`${d.date}-${d.title}`} className={`seed-deadline-card ${near ? 'near' : ''}`}>
...
export function HomeInsightPanels({onOpenTask}: {onOpenTask?: (id: string) => void} = {}) {
  ...
  return (
    <div className="seed-home-stack">
      ...
      <section className="seed-block warn" aria-label="思い込みで損しやすいこと">
        <div className="seed-block-head">
          <span className="eyebrow">DON'T BUY THESE LIES</span>
          <h3>思い込みで損しやすいこと</h3>
        </div>
      ...
      <section className="seed-block exclude" aria-label="もらえない制度と理由">
        <div className="seed-block-head">
          <span className="eyebrow">NOT PRIZES · EXCLUDE</span>
          <h3>もらえない制度と理由</h3>
          <p className="hint">{excludeMeta.description} · {excludeItems.length}件</p>
        </div>
      ...
      <section className="seed-block talk" aria-label="ふたりの会話のきっかけ">
        <div className="seed-block-head">
          <span className="eyebrow">TALK LINES</span>
          <h3>ふたりの会話のきっかけ</h3>
        </div>
  ...
  return (
    <section className="seed-block phases" aria-label="時期の区切りと出来事">
      <div className="seed-block-head">
        ...
        <h3>時期の区切りと出来事</h3>
        <p className="hint">
          M0 = {m0_definition} · 基準 {as_of} · {phases.length}区切り /{' '}
          {phases.reduce((n, p) => n + p.events.length, 0)}出来事
        </p>
      </div>
      <div className="seed-phase-list">
        {phases.map((ph) => (
          <details key={ph.id} className="seed-phase" open={ph.id === 'phase0'}>
            <summary>
              <Layers size={16} />
              <span>
