...
export function TaskForm({task:t,profile:p,record,onSave,busy,onDirty,hasBook:_hasBook,onProfile}:{task:Task,profile:Profile,record?:TaskRecord,onSave:(r:TaskRecord,mode?:'quiet'|'status')=>Promise<void>,busy:boolean,onDirty:(v:boolean)=>void,hasBook:boolean,onProfile:()=>void}){
 ...
 {t.type!=='investment'&&<AccordionItem value="money"><AccordionTrigger>給付・節約の金額を記録する（任意）</AccordionTrigger><AccordionContent><p className="hint">同じ制度の金額は、一つの項目だけに記録してください。見込額と受取額を重ねて計上しないよう、受取後は区分を変更します。</p>{t.amountNote&&<p className="hint">{t.amountNote}</p>}<div className="field-grid"><Choice label="金額の区分" value={r.moneyKind} onChange={v=>changeQuiet({moneyKind:v as TaskRecord['moneyKind'],amount:v==='none'?null:rRef.current.amount})} options={{none:'記録しない',received:'実際に受け取った給付・祝金',estimate:'まだ受け取っていない見込額',monthlySaving:'固定費の削減額（月額）',taxEstimate:'税負担の軽減見込額'}}/><div className="field"><Label htmlFor="task-amount">{r.moneyKind==='monthlySaving'?'削減額（円／月）':'金額（円）'}</Label><Input id="task-amount" type="number" inputMode="numeric" min={0} max={1000000000} step={1} disabled={r.moneyKind==='none'} value={r.amount??''} onChange={e=>changeQuiet({amount:e.target.value===''?null:Number(e.target.value)},350)} placeholder="不明なら空欄"/></div></div></AccordionContent></AccordionItem>}
